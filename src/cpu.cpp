 ...
 */
Cpu::Cpu(uint16_t cpu_num) : _base_operating_ratio(-1), _freq(1200)
{
    stringstream path;
...
Cpu::init_counters()
{
     ...
     *      not in halted state.
     */
    this->SetMsrBit(MSR_IA32_CORE_PERF_GLOBAL_CTRL, 
                    BIT_FIXED_ARCH_PERF_MONITOR_CTR_1);
    this->SetMsrBit(MSR_IA32_CORE_PERF_GLOBAL_CTRL, 
                    BIT_FIXED_ARCH_PERF_MONITOR_CTR_2);

    /* Enabling fixed counters for all rings via the fixed counter control register */
    this->SetMsrBit(MSR_IA32_FIXED_CTR_CTRL, 
...
Cpu::fini_counters()
{
    ...
    this->ClearMsrBit(MSR_IA32_CORE_PERF_GLOBAL_CTRL, 
                      BIT_FIXED_ARCH_PERF_MONITOR_CTR_1);
    this->ClearMsrBit(MSR_IA32_CORE_PERF_GLOBAL_CTRL, 
                      BIT_FIXED_ARCH_PERF_MONITOR_CTR_2);
    
    /* Disabling all ring levels for fixed-function counter 1 and 2 */
    this->ClearMsrBit(MSR_IA32_FIXED_CTR_CTRL, 
                      BIT_CONTROL_FIXED_COUNTER_1_LOW);
    this->ClearMsrBit(MSR_IA32_FIXED_CTR_CTRL, 
                      BIT_CONTROL_FIXED_COUNTER_1_HIGH);
    this->ClearMsrBit(MSR_IA32_FIXED_CTR_CTRL, 
...
/** 
 * Destructor of Cpu class. Closes file descriptors previously opened.
 */
Cpu::~Cpu() 
{
    if(this->is_open())
 ...
 */
int 
Cpu::ReadMsr(uint64_t regno, std::string  range, uint64_t *buff) 
{
    assert(buff != nullptr);
    if(!this->is_open())
        return -1;
...
int 
Cpu::WriteMsr(uint64_t regno, uint64_t pattern) 
{
    if(!this->is_open())
        return -1;
 ...
 */
int
Cpu::SetMsrBit(uint64_t regno, uint32_t bitno) 
{
 ...
 */
int
Cpu::ClearMsrBit(uint64_t regno, uint32_t bitno) 
{
    assert(bitno < regsize);
